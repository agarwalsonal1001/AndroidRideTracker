...
public class SelectExecutiveActivity extends BaseActivity implements ExecutivesRecyclerAdapter.OnItemClickListener {
    ...
    private void init() {
        try {
            ...
            fabAddExecutives.setVisibility(View.GONE);
                        ...
                        if (responseObj != null && responseObj.getStatus() != null) {
                            if (responseObj.getStatus().getStatusCode() == SUCCESS) {
                                if (responseObj.getExecutivesDetailsObjArrayList() != null && responseObj.getExecutivesDetailsObjArrayList().size() > 0) {
                                    ...
                                    executivesRecyclerAdapter = new ExecutivesRecyclerAdapter(((ArrayList) responseObj.getExecutivesDetailsObjArrayList()), SelectExecutiveActivity.this, SelectExecutiveActivity.this,false);
    ...
    public void onItemClick(View view, int position, ExecutivesDetailsObj object) {
        Intent callMapActivity = new Intent(getApplicationContext(), MapsActivity.class);
        callMapActivity.putExtra("ExecutivesDetailsObj", object);
